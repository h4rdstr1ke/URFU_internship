...
<form method="post" action="php/save_account.php" id="accountForm">
  ...
  <label>email</label>
  <input type="text" name="email" value="<?= htmlspecialchars($userData['email'] ?? '') ?>" placeholder="email" disabled
    required><br>
  <span class="error" id="emailError"></span><br>

  <label>Telegram</label>
  <input type="text" name="telegram" value="<?= htmlspecialchars($userData['telegram'] ?? '') ?>" placeholder="Telegram"
    disabled><br>
...
<button type="submit" id="editButton" <?= $workman_status == 1 ? 'style="display: none;"' : '' ?>>Редактировать</button>
  ...
  if ($status == 'На рассмотрении'): ?>
    <div class="status-notice">
      ...
      if ($result3 && $result3->num_rows > 0) {
        $result3->data_seek(0); // Сбрасываем указатель
        $first = true;
        while ($row3 = $result3->fetch_assoc()) {
          echo ($first ? '' : ', ') . htmlspecialchars($row3['title']);
          $first = false;
        }
      } else {
        echo 'нет данных';
